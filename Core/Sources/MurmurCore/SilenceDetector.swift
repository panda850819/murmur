...
public enum SilenceDetector {
    /// True when the recording at `wavURL` is silence: its RMS amplitude is
    /// below `rmsThreshold`, or the file is unreadable / has zero frames.
    /// Fails CLOSED (unreadable ⇒ silent): a clip we cannot even decode is
    /// not going to transcribe either, so "try again" is the right answer.
    ///
    ...
    public static func isSilent(wavURL: URL, rmsThreshold: Float = 0.005) -> Bool {
        guard let file = try? AVAudioFile(forReading: wavURL), file.length > 0 else {
            return true
        }
